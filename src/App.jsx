...
function Lightbox({ screenshots, startIndex, onClose }) {
  ...
  return (
    <div className="lightbox-overlay" onClick={onClose}>
      ...
      <button className="lb-arrow lb-prev" onClick={(e) => { e.stopPropagation(); prev() }} aria-label="Önceki">
        ‹
      </button>
      ...
      <button className="lb-arrow lb-next" onClick={(e) => { e.stopPropagation(); next() }} aria-label="Sonraki">
        ›
      </button>
  ...
  return (
    ...
    <div className="about-highlights">
            <div className="highlight-item">
              ...
              <div>
                <span className="hl-title">Backend & API</span>
                <span className="hl-desc">.NET 8, ASP.NET Core, REST, CQRS</span>
              </div>
            </div>
            <div className="highlight-item">
              <span className="hl-icon">⬡</span>
              <div>
                <span className="hl-title">Veritabanı</span>
                <span className="hl-desc">SQL Server, PostgreSQL, MongoDB, Redis</span>
              </div>
            </div>
            <div className="highlight-item">
              <span className="hl-icon">⬡</span>
              <div>
                <span className="hl-title">Mimari</span>
                <span className="hl-desc">Clean Architecture, Microservices, MVVM</span>
              </div>
            </div>
            <div className="highlight-item">
              <span className="hl-icon">⬡</span>
              <div>
                <span className="hl-title">Frontend & Mobil</span>
                <span className="hl-desc">React, Next.js, Flutter, WPF</span>
              </div>
            </div>
          </div>
        </div>
...
const WORKS = [
  {
    ...
    features: [
      'Barkod ile hızlı ürün tanıma',
      'Satış & satış geçmişi takibi',
      'Stok hareketi ve uyarı sistemi',
      'Fiyat teklifi oluşturma',
      'Kategori & ürün yönetimi',
      'Anlık dashboard istatistikleri',
    ],
    tech: ['WPF', 'ASP.NET Core', 'MVVM', 'SQL Server', 'Clean Architecture'],
    screenshots: [
      '/work1ss1.png','/work1ss2.png','/work1ss3.png','/work1ss4.png','/work1ss5.png',
      '/work1ss6.png','/work1ss7.png','/work1ss8.png','/work1ss9.png','/work1ss10.png',
      '/work1ss11.png','/work1ss12.png','/work1ss13.png','/work1ss14.png','/work1ss15.png',
      '/work1ss16.png','/work1ss17.png',
    ],
    badge: 'Ticari Ürün',
  },
  {
    ...
    features: [
      '72+ ürün & hiyerarşik kategori',
      'Sipariş takibi & durum yönetimi',
      'Müşteri & sipariş geçmişi',
      'Ürün bazlı indirim tanımlama',
      'Slider & kayan yazı yönetimi',
      'Tam işlem logu kaydı',
    ],
...
function WorkCard({ work, index }) {
  ...
  useEffect(() => {
    if (work.screenshots.length <= 1) return
    const t = setInterval(() => {
      setActiveImg(i => (i + 1) % work.screenshots.length)
    }, 3200)
  ...
  return (
    <>
      ...
      <div
        className={`work-card ${visible ? 'in-view' : ''} ${isEven ? 'reverse' : ''}`}
        ref={ref}
      ...
      >
          ...
          >
            ...
            {work.screenshots.map((src, i) => (
              <img
                key={src}
                src={src}
                alt={`${work.title} ekran ${i + 1}`}
                className={`work-img ${i === activeImg ? 'active' : ''}`}
              />
            ))}
            <div className="work-img-placeholder">
              <span>public/work{work.id}ss1.png</span>
            </div>
            <div className="work-img-dots" onClick={e => e.stopPropagation()}>
              {work.screenshots.map((_, i) => (
                <button
                  key={i}
                  className={`work-dot ${i === activeImg ? 'active' : ''}`}
        ...
        <div className="work-info">
          ...
          <div className="work-cta">
            ...
            {work.link && (
              <a href={work.link} target="_blank" rel="noreferrer" className="btn-outline">
                Siteyi Gör →
              </a>
            )}
          </div>
...
function ProjectCard({ project, index }) {
  ...
  return (
    <>
      {lightbox !== null && (
        <Lightbox screenshots={project.screenshots} startIndex={lightbox} onClose={() => setLightbox(null)} />
      )}
      <article className={`project-card ${visible ? 'in-view' : ''}`} ref={cardRef} style={{ transitionDelay: `${index * 0.08}s` }}>
        <div className="project-media">
          <div className="project-img-wrap" onClick={() => setLightbox(activeImg)} title="Büyütmek için tıkla">
            <div className="work-zoom-hint">🔍 Tıkla</div>
          ...
          <div className="project-thumbs" onClick={e => e.stopPropagation()}>
      ...
      <div className="contact-inner">
        ...
        <div className="contact-cards">
          <a href="mailto:aliyedibela07@gmail.com" className="contact-card">
            <span className="cc-icon">✉</span><span className="cc-label">E-posta</span><span className="cc-value">ali.yedibela@example.com</span>
          </a>
